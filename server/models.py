...
class Employee(db.Model):
    ...
    # Relationsihp mapping the mployee to related reviews
    reviews = db.relationship('Review', back_populates="employee", cascade='all, delete-orphan')
    
    # Relationship mapping employee to related onboarding (has one)
...
class Onboarding(db.Model):
    ...
    # Foreign key to store the employee id (Belongs to)
...
class Review(db.Model):
    ...
    # Foreign key stores the Employee id; in Flask SQL-Alchemy it is stored like so:
    employee_id = db.Column(db.Integer, db.ForeignKey('employees.id'))
